...
- **标签系统** — 灵活的文章分类
- **评论系统** — 支持嵌套回复、邮件通知、AI 辅助审核与上下文化评论审核；评论入库时记录作者 IP / 归属地（ip-api.com，中文市级 + 运营商/ASN/经纬度），前台展示归属地（不暴露 IP），管理后台展示完整信息便于识别机房/代理流量
...
| `author_region` | ip-api.com（中文，含国家/省/市 + 运营商/ASN/经纬度），4 秒超时，失败容忍 | 前台 + 管理后台    |

> 归属地查询走 ip-api.com 免费接口（HTTP，45 次/分钟），国内可到市级。
> 前台只显示「|」之前的归属地部分（如「中国 / 广东省 / 深圳市」），不暴露运营商/ASN/经纬度；管理后台展示完整信息。
> 查询失败时 `author_region` 为空，评论照常创建。旧评论的 region 字段保留入库时的值。
